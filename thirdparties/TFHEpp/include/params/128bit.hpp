...
struct lvl0param {
    ...
    static constexpr std::uint32_t n = 667;  // dimension
...
struct lvl1param {
    static constexpr std::uint32_t nbit = 10;  // dimension must be a power of 2 for ease of polynomial
             // multiplication.
    static constexpr std::uint32_t n = 1 << nbit;  // dimensi on
    static constexpr std::uint32_t k = 1;
    ...
    using T = uint32_t;                                 // Torus representation
    static constexpr T μ = 1U << (32-3);
    static constexpr uint32_t plain_modulus = 2;
    static constexpr double Δ =
        static_cast<double>(1ULL << std::numeric_limits<T>::digits) /
        plain_modulus;//(1ULL 64 bit) 
};
...
struct lvl2param {
    ...
    static constexpr T μ = 1ULL << ( 64-7 );
...
struct lvl20param {
    ...
    static constexpr std::uint32_t basebit =
        6;  // how many bit should be encrypted in keyswitching key
    static const inline double α = lvl0param::α;  // key noise
