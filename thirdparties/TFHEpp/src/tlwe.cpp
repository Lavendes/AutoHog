/*
 * September 2022 - Modified by Ran Mao
*/
...
template <class P>
TLWE<P> bootsSymEncrypt(const uint8_t &p,
                                     const SecretKey &sk)
{
    TLWE<P> c;
    //std::cout<<fixed<<setprecision(20) << P::μ<<" "<<P::μ/pow(2.0,32)<<std::endl;
    c= tlweSymEncrypt<P>(p ? P::μ : -P::μ, P::α, sk.key.get<P>());
    return c;
}
#define INST(P)                                       \
    template TLWE<P> bootsSymEncrypt<P>( \
        const uint8_t &p, const SecretKey &sk)
TFHEPP_EXPLICIT_INSTANTIATION_TLWE(INST)
#undef INST


template <class P>
uint8_t bootsSymDecrypt(const TLWE<P> &c,
                                     const SecretKey &sk)
{
    uint8_t p;
    p = tlweSymDecrypt<P>(c, sk.key.get<P>());
    return p;
}
#define INST(P)                                       \
    template uint8_t bootsSymDecrypt<P>( \
        const TLWE<P> &c, const SecretKey &sk)
TFHEPP_EXPLICIT_INSTANTIATION_TLWE(INST)
#undef INST
}  // namespace TFHEpp
